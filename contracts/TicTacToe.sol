...
contract TicTacToe is ERC1155 {
    ...
    function play(uint256 gameId, uint row, uint column) public
    {
        ...
        {
            ERC20(g.token).transferFrom(address(this), g.player0, g.balance0);
            ERC20(g.token).transferFrom(address(this), g.player1, g.balance1);
            g.balance0 = 0;
            g.balance1 = 0;
            g.turn = 0;
        ...
        {
            if(player == 0)
                ERC20(g.token).transferFrom(address(this), g.player0, g.balance0 + g.balance1);
            else
                ERC20(g.token).transferFrom(address(this), g.player1, g.balance0 + g.balance1);
